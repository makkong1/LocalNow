...
**구현 난이도 주의:** 단순 URL 매핑 추가가 아니다. 모바일 `ChatRoomSummaryResponse` 타입이 요구하는 필드는 아래와 같다.
...
interface ChatRoomSummaryResponse {
  roomId: number;
  requestId: number;
  requestType: RequestType;       // help_requests JOIN 필요
  partnerName: string;            // users JOIN 필요
  lastMessagePreview: string | null;  // chat_messages 집계 쿼리 필요
  lastMessageAt: string | null;       // chat_messages 집계 쿼리 필요
}
```

반면 백엔드의 `ChatRoomResponse`는 `id, requestId, travelerId, guideId, createdAt`만 담고 있어 3-way JOIN 또는 서브쿼리 + 신규 DTO `ChatRoomSummaryResponse.java` + `ChatService.getRoomsForUser()` 메서드가 모두 필요하다.

- **결론:** 채팅 목록 탭(`ChatListScreen`)은 UI는 있으나, **연동 완료 상태로 보기 어렵다**. 로드맵 1번(읽음) 전에 **목록 API 계약 구현** 또는 클라이언트를 기존 엔드포인트 조합으로 바꾸는 작업이 선행된다.

### 2.2 `ChatListScreen` 오류 침묵 버그

`ChatListScreen.tsx`에 `isError` 분기가 없다.

```typescript
// 현재 코드 (ChatListScreen.tsx)
const { data: rooms, isLoading } = useChatRooms();

if (!rooms || rooms.length === 0) {
  return <Text>확정된 매칭이 없습니다</Text>  // API 오류도 여기로 빠짐
}
```

백엔드 `/chat/rooms`가 미구현이므로 현재 항상 오류를 반환하지만, 사용자에게는 "확정된 매칭이 없습니다"로 표시된다. **API 오류와 실제 빈 목록을 구분할 방법이 없다.** `isError` 분기 추가가 필요하다.
...
- 문서 갱신 시 **탭=목록 + 백엔드 `/chat/rooms` 공백 + 오류 침묵 버그**를 함께 적는 편이 정확하다.
...
### 3.2 `ChatController` 경로 소유권 혼재 🟡 (구조적 리스크)

```java
// ChatController.java — 클래스 레벨 @RequestMapping 없음
@GetMapping("/requests/{requestId}/room")   // /requests/* 네임스페이스 점유

// RequestController.java
@RequestMapping("/requests")               // 동일 접두사 점유
```

현재는 충돌하지 않지만 두 컨트롤러가 `/requests/` 경로를 공유한다. `ChatController`에 엔드포인트를 추가할 때 의도치 않은 라우팅 충돌 위험이 있다. `/chat/rooms` 구현 시 `ChatController`에 `@RequestMapping("/chat")`을 추가하고 기존 경로를 정리하는 것이 적절하다.

### 3.3 `MatchNotificationListener` — `match.offer.created` 발행자 불명확 🟡

```java
// MatchNotificationListener.java
switch (routingKey) {
    case "match.offer.created" -> handleOfferCreated(payload);  // guideIds 배열 기대
    case "match.offer.accepted" -> handleOfferAccepted(payload);
    case "match.confirmed" -> handleMatchConfirmed(payload);
}
```

`MatchService.accept()`는 `"match.offer.accepted"`를 발행하지만 `"match.offer.created"`(여러 가이드에게 신규 요청 알림)를 발행하는 코드는 `MatchService`에 없다. 발행 주체가 `MatchDispatcher`라면 해당 파일에서 별도 확인이 필요하다. 이 경로가 동작하는지 현재 불확실하다.
...
| 항목 | 근거 | 심각도 |
| --- | --- | --- |
| **`GET /chat/rooms` 구현 또는 모바일 수정** | §2.1 — 현재 연동 불일치. 단순 URL 추가가 아닌 3-way JOIN + 신규 DTO 작업. | 🔴 |
| **PG 이중 호출 경쟁 조건** | §3.1 — `createIntent()` 락 없음. 실 PG 연동 전 블로커. | 🔴 |
| **`ChatListScreen` 오류 침묵** | §2.2 — `isError` 분기 없어 API 오류가 빈 목록으로 표시됨. | 🟡 |
| **요청 취소 REST** | `toCancelled()`만 존재, API 없음. | 🟡 |
| **`IN_PROGRESS` 사용 여부 결정** | 호출부 없음. 제거·문서화·"서비스 시작" API 중 택일. | 🟡 |
| **`start_at` 정책 명시** | 필드만 있고 매칭 타이밍에 미반영. | 🟡 |
| **`ChatController` 경로 혼재** | §3.2 — `/requests/*` 네임스페이스 공유. `/chat/rooms` 구현 시 함께 정리. | 🟡 |
| **`match.offer.created` 발행자 확인** | §3.3 — `MatchDispatcher`에서 발행하는지 별도 확인 필요. | 🟡 |
| **환불 후 completedCount 오염** | §3.4 — 환불 시 HelpRequest 상태·가이드 통계 정책 결정 필요. | 🟡 |
...
0-a. **즉시 (5분):** `ChatListScreen` `isError` 분기 추가 — API 오류가 빈 목록으로 위장되는 것 차단.
0-b. **선행:** `GET /chat/rooms` 백엔드 추가 (3-way JOIN + DTO + `ChatController` 경로 정리) — 그렇지 않으면 1번·채팅 탭 검증이 불가능하다.
1. **2 — GEO heartbeat/TTL** (서버 TTL/보조키 + RN 주기 duty)
2. **4 — notification 테이블 + API**
3. **3 — 결제 상태 세분화** + §3.1 PG 이중 호출 경쟁 조건 수정 병행
4. **1 — 채팅 읽음** (0-b 완료 후)
5. **5 — 서비스 경계 정리**
6. **빠른 승리:** 요청 취소 REST + UI, `IN_PROGRESS` 용도 정리, `match.offer.created` 발행자 확인
